...
python steering.py
```
For vLLM multiprocessing, we recommend the step-by-step workflow below so vector generation and vector application run in separate Python processes.
...
- **vLLM Support**: generate steering vectors with vLLM in in-process mode, and apply activation-addition steering vectors with vLLM worker hooks for multiprocessing inference.
...
You can choose to use vLLM in the generate or apply stages of different steering methods. The two stages use different execution modes:

- **Generate Vector**: methods that read hidden activations run vLLM in in-process mode so EasyEdit2 can capture residual-stream activations.
- **Apply Vector**: activation-addition methods can use vLLM multiprocessing. EasyEdit2 installs worker-side hooks that add steering vectors to decoder-layer residual streams during inference.
...
`*` These methods need activation capture during vector generation, so EasyEdit2 disables vLLM V1 multiprocessing for the generate stage even when `vllm_enable: true`. For stable multiprocessing acceleration, generate vectors first and then apply them in a separate run.
...
For activation-addition methods (`caa`, `vector_prompt`, `sta`, and `sae_feature`), vLLM application uses worker-side hooks. The hooks add the steering vector to the configured decoder-layer residual stream inside the vLLM worker process, so multiprocessing inference can still be accelerated.
...
vllm_enable_v1_multiprocessing: True   # Recommended for apply/generation with worker hooks
vllm_enforce_eager: True               # Recommended for runtime steering hooks
...
When `vllm_enable: true`, vector generation for activation-capture methods such as CAA, STA, and Vector Prompt automatically runs with `vllm_enable_v1_multiprocessing: false`, because their generators need to read hidden activations in the current Python process. The apply/generation stage can use `vllm_enable_v1_multiprocessing: true`; EasyEdit2 will inject the steering vectors inside vLLM workers.

For vLLM multiprocessing, the most stable workflow is to run vector generation and vector application as separate commands:
...
This avoids creating and destroying multiple vLLM engines inside the same Python process, which can leave GPU memory or distributed state behind on some vLLM/PyTorch versions.
